...
private:
	...
	//Queue for presentation to window
	...
	bool checkDeviceExtensionSupport(VkPhysicalDevice device)
	{
		...
		vkEnumerateDeviceExtensionProperties(device, nullptr, &extensionCount, avalibleExtensions.data());
	...
	{
		//Check queue support for both drawing and presentation to surface? 
	...
	void createSwapChain()
	{
		...
		createInfo.clipped = VK_TRUE; //Plane clipping
	...
	void createDescriptorSetLayout()
	{
		...
		{
			throw std::runtime_error("failet to create descriptor set layout!");
		}
	}

	void createGraphicsPipeline()
	{
		//Create shader moduels
		auto vertShaderCode = readFile("shaders/vert.spv");
		auto fragShaderCode = readFile("shaders/frag.spv");

		VkShaderModule verShaderModule = createShadeModule(vertShaderCode);
		VkShaderModule fragShaderModule = createShadeModule(fragShaderCode);

		//Create shader stages
		VkPipelineShaderStageCreateInfo vertShaderStageInfo = {};
		vertShaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
		vertShaderStageInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
		vertShaderStageInfo.module = verShaderModule;
		vertShaderStageInfo.pName = "main"; //Shader entry point function
		...
		{
			throw std::runtime_error("failet to create graphics pipeline!");
		}

		vkDestroyShaderModule(device, verShaderModule, nullptr);
	...
	VkFormat findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
	{
		...
		throw std::runtime_error("failet to find suported format!");
	}
	...
	void createTextureSampler()
	{
		...
		{
			throw std::runtime_error("failet to create texture sampler!");
		}
		...
		VkBuffer& buffer, VkDeviceMemory& bufferMemory)
	{
		...
		{
			throw std::runtime_error("failet to create buffer!");
		}
	...
	void createCommandBuffers()
	{
		...
		{
			throw std::runtime_error("failet to allocate command buffers");
		}
	...
	void drawFrame()
	{
		...
		{
			throw std::runtime_error("failet to present swap chain image!");
		}
	...
	uint32_t findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
	{
		...
		{
			if (typeFilter & (1 << i) && (memProperties.memoryTypes[i].propertyFlags & properties) == properties)  //!?!?!? Look more into this bitwise stuff
			{
				return i;
			}
		}
